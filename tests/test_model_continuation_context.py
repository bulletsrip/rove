...
class ModelContinuationContextTest(unittest.TestCase):
    ...
    def test_no_progress_is_explicit_feedback_for_replanning(self):
        captured = {}
        ...
        def request_json(_url, _key, body):
            ...
            return {
                ...
                "answers": {
                    "operation": {
                        "choice": "WAIT",
                        "probabilities": {"WAIT": 1.0, "DONE": 0.0, "BLOCKED": 0.0},
                        "confidence": 1.0,
                    }
                },
            }
        ...
        history = [{"action": "Search", "kind": "click", "page_changed": False}]

        with patch.object(model, "post_json", side_effect=request_json), patch.dict("os.environ", {"TYPESAFE_API_KEY": "test-key"}):
            model.choose(page, "Find the result", history)

        self.assertEqual(captured["body"]["state"]["no_progress"]["action"], "Search")
