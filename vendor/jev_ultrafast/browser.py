...
def english_google_url(url):
    ...
    if not any(key.lower() == "hl" for key, _ in query):
        query.append(("hl", "en"))
...
class Browser:
    def __init__(self, url):
        ...
        self.call("Emulation.setLocaleOverride", localeId="en-US")
        self.call("Network.setExtraHTTPHeaders", headers={"Accept-Language": "en-US,en;q=0.9"})
        self.call("Page.bringToFront")
        self.call("Emulation.setFocusEmulationEnabled", enabled=True)
        self.call("Page.navigate", url=english_google_url(url))
...
def browser_operation(request):
    ...
    if request.get("screenshot", True):
        try:
            ...
            if "Page.captureScreenshot" not in str(exc):
                raise
        else:
            if screenshot:
