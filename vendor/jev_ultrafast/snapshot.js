(() => {
  ...
  for (const e of document.querySelectorAll('*')) {
    ...
    scrollContainers.push([identity(e),Math.round(e.scrollTop),Math.round(e.scrollLeft),
      Math.round(e.scrollHeight),Math.round(e.scrollWidth),Math.round(e.clientHeight),Math.round(e.clientWidth)]);
  }
  const scroll={y:scrollY,height,containers:scrollContainers};
  const page_key=cache.pageKey(), guards={};
  for (const a of actions) if (!(a.node in guards)) guards[a.node]=cache.guard(cache.nodes.get(a.node));
  // Compare meaning and identity. Geometry is always resolved and hit-tested just before input.
  const semantics=actions.map(({rect,...action})=>action);
  const marker=[performance.timeOrigin,location.href,scrollX,scrollY,innerWidth,innerHeight,
    document.title,text,semantics,page_key[6],scroll];
