...
Supported operations are `CLICK`, `TYPE_TEXT`, `SELECT`, `SCROLL_UP`, `SCROLL_DOWN`, `WAIT`, `DONE`, and `BLOCKED`.
...
| Private bind address | The host address configured in `.env` where Docker publishes the private service ports. |
...
```bash
git clone --branch pre-release https://github.com/bulletsrip/rove.git
cd rove
cp .env.example .env
chmod 600 .env

# Edit .env with your own keys and the host's private bind address.
${EDITOR:-vi} .env
...
```text
http://<private-host-address>:8080
```

The task UI is on port `8080`. Port `6080` is the standalone noVNC fallback/debug view. Ports are published only on the configured private address; do not change the Compose bindings to `0.0.0.0` unless you intentionally want to expose the browser runtime.

Useful diagnostics:

```bash
sudo docker compose logs --tail=100
curl http://<private-host-address>:8080/api/status
```
